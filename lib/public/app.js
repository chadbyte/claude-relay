...
// --- DOM refs ---
  ...
  // --- Stream smoothing: buffer incoming deltas and drain character-by-character ---
  var streamBuffer = "";
  var streamDrainTimer = null;
  var streamRenderTimer = null;
  var streamCursorEl = null;
  ...
  function drainStreamTick() {
    if (!currentMsgEl || streamBuffer.length === 0) {
      streamDrainTimer = null;
      scheduleStreamRender();
      return;
    }

    // Adaptive: drain faster when buffer grows to avoid falling behind
    var n = 1;
    var len = streamBuffer.length;
    if (len > 150) { n = Math.ceil(len / 8); }
    else if (len > 60) { n = 3; }
    else if (len > 25) { n = 2; }
    ...
    // Fast path: append to a cursor span (cheap DOM op, no markdown parse)
    var contentEl = currentMsgEl.querySelector(".md-content");
    if (!streamCursorEl || !contentEl.contains(streamCursorEl)) {
      streamCursorEl = document.createElement("span");
      contentEl.appendChild(streamCursorEl);
    }
    streamCursorEl.textContent += chunk;

    scheduleStreamRender();
    scrollToBottom();

    streamDrainTimer = setTimeout(drainStreamTick, 5);
  }
  ...
  function flushStreamBuffer() {
    if (streamDrainTimer) { clearTimeout(streamDrainTimer); streamDrainTimer = null; }
