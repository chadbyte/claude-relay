...
// --- Commands registry ---
var commands = [
  { id: "create-mate", label: "New Mate", desc: "Create a new AI teammate with Clay", icon: "user-plus", action: "createMate" },
...
function activateItem(entry) {
  if (entry.type === "command") {
    ...
    closeCommandPalette();
    if (entry.data.mateId) {
      showHomeHub();
      openHomeChat(entry.data.mateId);
    }
  ...
  } else if (entry.type === "mate") {
    closeCommandPalette();
    showHomeHub();
    openHomeChat(entry.data.id);
