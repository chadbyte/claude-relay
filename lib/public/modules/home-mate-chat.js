...
function handleActionsMenuKeydown(event) {
  ...
  if (!actionsMenu || (event.key !== "ArrowDown" && event.key !== "ArrowUp" && event.key !== "Home" && event.key !== "End")) return;
  var items = actionsMenu.querySelectorAll('[role="menuitem"]');
...
function showActionsMenu(anchor, mate, mateName) {
  ...
  addActionsMenuItem(menu, "settings-2", "Mate settings", function () {
    openHomeMateProperty("settings", mate.id, mateName);
  });
  var divider = document.createElement("div");
  divider.className = "home-mate-actions-menu-divider";
  ...
  requestAnimationFrame(function () {
    positionActionsMenu(anchor, menu);
...
export function openHomeChat(mateId) {
  ...
  store.set({ homeChatMateId: mateId, homeChatSessionId: preferredSession });
...
export function openHomeConversation(mateId, sessionId) {
  ...
  closeActionsMenu();
  if (store.get('homeChatMateId') && store.get('homeChatMateId') !== mateId) closeHomeMateBackstage();
  store.set({ homeChatMateId: mateId, homeChatSessionId: sessionId });
  rememberHomeMate(mateId);
...
export function openHomeMateActions(anchor) {
  var mate = getMate(store.get('homeChatMateId'));
  if (!mate || !anchor) return;
  if (actionsMenu) closeActionsMenu();
  else showActionsMenu(anchor, mate, getMateName(mate));
}
...
export function resumeHomeChat() {
  ...
  if (sessionId) sendMessage({ type: "home_mate_session_open", mateId: mateId, sessionId: sessionId });
  else sendMessage({ type: "home_mate_open", mateId: mateId });
}
...
export function handleHomeMateHistory(msg) {
  ...
  store.set({ homeChatSessionId: msg.sessionId || null });
...
export function handleHomeMateError(msg) {
  ...
  if (msg.code === "session_not_found") {
    ...
    sendMessage({ type: "home_mate_open", mateId: msg.mateId });
    return;
  }
  if (streamingText) messages.push({ role: "assistant", text: streamingText, ts: Date.now() });
  streamingText = "";
