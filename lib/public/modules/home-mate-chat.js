...
function renderHeader(mate, mateName) {
  ...
  if (mate) {
    ...
    actions.className = "home-mate-chat-actions";
    var utilities = [
      { kind: "memory", icon: "brain", label: "View memory" },
      { kind: "knowledge", icon: "book-open", label: "View knowledge" },
    ];
    for (var i = 0; i < utilities.length; i++) {
      (function (utility) {
        var button = document.createElement("button");
        button.type = "button";
        button.className = "home-mate-chat-utility";
        button.title = utility.label;
        button.setAttribute("aria-label", utility.label);
        button.innerHTML = iconHtml(utility.icon);
        button.addEventListener("click", function () {
          openHomeMateProperty(utility.kind, mate.id, mateName);
        });
        actions.appendChild(button);
      })(utilities[i]);
    }
    var debateButton = document.createElement("button");
    debateButton.type = "button";
    debateButton.className = "home-mate-chat-utility";
    debateButton.title = "Start debate";
    debateButton.setAttribute("aria-label", "Start debate");
    debateButton.innerHTML = iconHtml("mic");
    ...
    actions.appendChild(debateButton);
