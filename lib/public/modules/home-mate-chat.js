...
function submitMessage() {
  ...
  if (!mateId || !text || streaming || !hasCommittedSessionModel()) return false;
...
function bindComposer() {
  ...
  sessionModelChooseEl.addEventListener("click", function () { openHomeMateAction("model", { sessionId: store.get('homeChatSessionId') }); });
...
export function renderHomeChat() {
  ...
  inputEl.disabled = !mateId || !hasCommittedSessionModel();
...
export function openHomeMateAction(kind, options) {
  var mate = getMate(store.get('homeChatMateId'));
  if (!mate) return false;
  if (kind === "memory" || kind === "knowledge" || kind === "model" || kind === "settings") {
    openHomeMateProperty(kind, mate.id, getMateName(mate), options);
    return true;
  }
