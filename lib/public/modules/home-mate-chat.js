...
var respondToDebateTranscript = createHomeDebateResponder(sendMessage, function () { return { mateId: store.get('homeChatMateId'), sessionId: store.get('homeChatSessionId'), requestId: activeSessionRequestId }; });
...
export function renderHomeChat() {
  ...
  inputEl.disabled = debateLaunching || !mateId || awaitingQuestion || !hasCommittedSessionModel();
  inputEl.placeholder = debateLaunching ? "Preparing debate…" : (awaitingQuestion ? "Answer the question above to continue" : (mateId ? "Message " + mateName : "Message"));
  sendBtn.disabled = debateLaunching || !mateId || streaming || awaitingQuestion || !hasCommittedSessionModel() || !inputEl.value.trim();
  renderSessionModel();
  if (animateSwitch) {
...
export function handleHomeDebateTranscript(msg) {
  ...
  if (msg.type === "home_debate_proposal" || msg.type === "home_debate_question") messages = finalizeHomeAssistant(messages, streamingText, "", Date.now());
  ...
  if (msg.type === "home_debate_proposal" || msg.type === "home_debate_question") { streaming = false; streamingText = ""; }
  renderHomeChat();
