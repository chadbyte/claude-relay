...
export function syncHomeDebatePlanningPending(msg) {
  ...
  if (!msg || msg.debatePhase !== "planning") return;
...
export function createHomeDebateQuestionCard(message, respond) {
  ...
  answerControl.placeholder = freeform ? "Describe the topic in your own words" : "Type another answer";
...
export function normalizeHomeTranscript(raw) {
  return (raw || []).filter(function (message) {
    return message && ((message.role === "assignment" && message.assignment) || (message.role === "proposal" && message.proposal) || (message.role === "question" && message.toolId) || ["debate_header", "debate_turn", "debate_user", "debate_tool_decision"].indexOf(message.role) !== -1 || ((message.role === "user" || message.role === "assistant") && typeof message.text === "string"));
...
export function createHomeDebateResponder(sendMessage, getContext, onChange) {
  return function respond(message, action, opener, answers, modelOverrides) {
    ...
    var payload = message.role === "question" ? { type: "home_debate_question_response", toolId: message.toolId, answers: answers || {} } : { type: "home_debate_proposal_response", proposalId: message.proposal.proposalId, action: action, modelOverrides: Array.isArray(modelOverrides) ? modelOverrides : [] };
