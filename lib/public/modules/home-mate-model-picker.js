// Vendor-aware Mate model chooser hosted by the Home backstage.
...
function findFocusTarget(kind, value) {
  if (kind === "retry") return document.querySelector("#home-dock-content .home-mate-model-retry");
  if (kind === "selection-status") return document.querySelector("#home-dock-content .home-mate-model-selection-status");
  var selector = kind === "vendor" ? "#home-dock-content [data-home-mate-vendor]" : "#home-dock-content [data-home-mate-model]";
...
export function resetHomeMateModelPicker(mateId, mateName, mate) {
  activeMateId = mateId;
...
function selectMateModel(model, rerender) {
  ...
  if (send({ type: "home_mate_model_set", mateId: activeMateId, vendor: state.vendor, model: model, requestId: selectionRequestId })) return;
  selectionRequestId = null;
  selectionModel = null;
  state.error = "Clay is offline. Reconnect and try again.";
...
export function renderHomeMateModelPicker(body, rerender) {
  ...
  note.textContent = "Used for new conversations. Existing conversations keep their current vendor and model.";
...
export function applyHomeMateModelResult(msg) {
  ...
  state.error = "";
  window.dispatchEvent(new CustomEvent("clay:home-mate-model-confirmed", { detail: { mateId: msg.mateId, vendor: state.mateVendor, model: state.mateModel } }));
  return true;
}
