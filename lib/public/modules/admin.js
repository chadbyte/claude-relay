...
function renderInvitesTab(body) {
  ...
  html += '<button class="admin-action-btn admin-action-btn-secondary" id="admin-create-invite">' + iconHtml("link") + ' Copy Link</button>' +
    '</div></div>';
...
function renderSmtpTab(body, cfg) {
  ...
  if (hasConfig) {
    html += '<div class="admin-smtp-status admin-smtp-status-ok">' + iconHtml("check-circle") + ' SMTP configured — email login and invites are active</div>';
  } else {
