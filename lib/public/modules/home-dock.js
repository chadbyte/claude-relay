...
function syncDockTrigger() {
  ...
  if (label) label.textContent = backstageView ? backstageView.label : (store.get('dockLibraryOpen') ? "Capsules" : (tool ? tool.name : "Tools"));
...
export function closeHomeDock() {
  ...
  if (homeControl && homeControl.getClientRects().length) homeControl.focus();
}
