...
function syncDockTrigger() {
  ...
  var capsules = document.getElementById("home-sidebar-capsules");
  var activity = document.getElementById("home-sidebar-capsules-activity");
  var tool = activeTool();
  var hasActivity = store.get('dockHasActivity') === true;
  if (label) label.textContent = store.get('dockLibraryOpen') ? "Capsules" : (tool ? tool.name : "Tools");
  if (activity) activity.classList.toggle("is-active", hasActivity);
  if (capsules) {
    capsules.setAttribute("aria-expanded", store.get('dockOpen') === true ? "true" : "false");
    capsules.setAttribute("aria-label", hasActivity ? "Capsules, new activity" : "Capsules");
  }
...
export function openHomeDock(toolId) {
  ...
  setDockState({ dockActiveToolId: requested, dockOpen: true, dockFocus: false, dockLibraryOpen: false, dockHasActivity: false, dockActivityToolId: null }, true);
}
...
export function renderDock() {
  ...
  var selected = libraryOpen ? null : activeTool();
