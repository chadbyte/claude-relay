...
function renderWeekView() {
  ...
  // Current time indicator (inside scheduler-week-view for correct % positioning)
  ...
  for (var d = 0; d < 7; d++) {
    ...
    for (var ri = 0; ri < records.length; ri++) {
      if (records[ri].source === "ralph" || records[ri].source === "schedule") continue;
      taskCount++;
    }
    html += '<div class="scheduler-week-footer-cell">';
    if (taskCount > 0) html += '<span class="scheduler-week-task-badge">' + taskCount + ' Tasks</span>';
    html += '</div>';
  }
  html += '</div>';
