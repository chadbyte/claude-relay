...
export function processMessage(msg) {
    ...
    switch (msg.type) {
      ...
      case "history_done":
        ...
        if (_vtw2 && _hTotal > 0) _vtw2.classList.add("hidden");
      ...
      case "model_info":
        store.set({ currentModel: msg.model || store.get('currentModel'), currentModels: msg.models || [] });
        if (msg.vendor) store.set({ currentVendor: msg.vendor });
        if (msg.availableVendors) store.set({ availableVendors: msg.availableVendors });
      ...
      case "session_switched":
        ...
        store.set({ activeSessionId: msg.id, cliSessionId: msg.cliSessionId || null });
        if (msg.vendor) store.set({ currentVendor: msg.vendor });
        // Show vendor toggle only for new sessions (no history)
        var _vtw = document.getElementById("vendor-toggle-wrap");
        if (_vtw) _vtw.classList.toggle("hidden", !!msg.hasHistory);
