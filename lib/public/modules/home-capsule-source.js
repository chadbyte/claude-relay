...
function closeInspector(host) {
  ...
  if (host.sourceButton && host.sourceButton.isConnected) host.sourceButton.focus({ preventScroll: true });
}
...
export function mountCapsuleHostControls(toolId, definition, chromeRoot, contentRoot) {
  ...
  sourceButton.className = "home-capsule-source-action";
  sourceButton.setAttribute("aria-label", "View source for " + definition.manifest.name);
  sourceButton.title = "View Capsule source";
  sourceButton.innerHTML = iconHtml("file-code-2") + '<span class="home-capsule-source-label">Source</span>';
  sourceButton.addEventListener("click", function () { openInspector(hosts[toolId]); });
  toolbar.appendChild(sourceButton);
  var host = { toolId: toolId, definition: definition, root: contentRoot, sourceButton: sourceButton, access: null, accessStatus: null, inspector: null, sourceRequestId: null };
  hosts[toolId] = host;
