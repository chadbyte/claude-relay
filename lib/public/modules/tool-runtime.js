...
export function createToolRuntime(config) {
  ...
  function sendLlmOperation(msg) {
    ...
    var ws = getWs();
    if (!ws || ws.readyState !== 1) {
      worker.postMessage({ type: "llm_result", seq: msg.seq, error: "Tool LLM service is unavailable." });
      return;
    }
    var requestId = config.toolId + ":llm:" + workerGeneration + ":" + msg.seq;
  ...
  function handleWorkerMessage(event) {
    ...
    if (msg.type === "state") {
      ...
      if (!shouldInitialize && readyWaiters.length > 0) {
        isReady = true;
        var waiters = readyWaiters;
        readyWaiters = [];
        for (var wi = 0; wi < waiters.length; wi++) waiters[wi].resolve(state);
      }
      if (shouldInitialize) {
        initialActionSent = true;
        dispatchAction(config.initialAction, {}, "user").catch(function () {});
      } else {
        isReady = true;
      }
