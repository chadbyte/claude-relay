...
function toggleDmUserPicker(anchorEl) {
  ...
  function renderMatesDisabledLayout() {
    ...
    promoText.textContent = "AI teammates with long-term memory that persists across sessions. Mention them with @ in any session, or build your own.";
    promo.appendChild(promoText);

    var cta = document.createElement("button");
    cta.type = "button";
    cta.className = "dm-picker-mates-promo-cta";
    cta.innerHTML = iconHtml("settings") + " <span>Click here to enable Mates</span>";
  ...
  if (matesEnabled) {
    ...
    renderPickerList("");
    // Disabled layout doesn't call renderMatesList (which is what triggers
    // refreshIcons in the enabled path), so the CTA's lucide icon would
    // stay as an unconverted <i data-lucide="settings"> tag. Trigger the
    // converter explicitly here.
    refreshIcons();
    searchInput.addEventListener("input", function () {
