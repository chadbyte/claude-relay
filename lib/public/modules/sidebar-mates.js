...
export function renderUserStrip(allUsers, onlineUserIds, myUserId, dmFavorites, dmConversations, dmUnread, dmRemovedUsers, matesList) {
  ...
  // Mates no longer live in the sidebar: home (the C button) is where they
  // live now, listed next to the board they take work from. The strip is
  // people only. Kept as an empty list so the code below stays intact for
  // the mobile chip rail, which still renders mates from the same cache.
  var favoriteMates = [];
