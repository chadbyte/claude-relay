...
export function initServerSettings(appCtx) {
  ...
  // Auto-continue on rate limit toggle
  var autoContinueToggle = document.getElementById("settings-auto-continue");
  if (autoContinueToggle) {
    autoContinueToggle.addEventListener("change", function () {
      var ws = ctx.ws;
      if (ws && ws.readyState === 1) {
        ws.send(JSON.stringify({ type: "set_auto_continue", value: this.checked }));
      }
    });
  }
...
export function updateDaemonConfig(config) {
  ...
  var autoContinueToggle = document.getElementById("settings-auto-continue");
  if (autoContinueToggle) autoContinueToggle.checked = !!config.autoContinueOnRateLimit;
...
export function handleAutoContinueChanged(msg) {
  var autoContinueToggle = document.getElementById("settings-auto-continue");
  if (autoContinueToggle) autoContinueToggle.checked = !!msg.autoContinueOnRateLimit;
}
