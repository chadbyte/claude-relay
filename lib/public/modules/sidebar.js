...
export function initSidebar(_ctx) {
  ...
  if (toolsToggle && toolsSection) {
    // Restore saved state
    try {
      if (localStorage.getItem("sidebar-tools-collapsed") === "1") {
        toolsSection.classList.add("collapsed");
      }
    } catch (e) {}
    toolsToggle.addEventListener("click", function () {
      var collapsed = toolsSection.classList.toggle("collapsed");
      try { localStorage.setItem("sidebar-tools-collapsed", collapsed ? "1" : ""); } catch (e) {}
    });
  }
  ...
  if (mateResizeHandle && mateSidebarCol) {
    ...
    function onMateResizeEnd() {
      ...
      try { localStorage.setItem("sidebar-width", mateSidebarCol.style.width); } catch (e) {}
    }
