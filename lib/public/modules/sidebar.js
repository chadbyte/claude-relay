...
export function initSidebar(_ctx) {
  ...
  var resumeIdInput = document.getElementById("resume-session-id-input");
  var resumeIdBtn = document.getElementById("resume-session-id-btn");
  ...
  function openResumeModal() {
    ...
    if (resumeIdBtn) resumeIdBtn.disabled = true;
    if (ctx.ws && ctx.connected) {
  ...
  function importSessionById() {
    var sid = resumeIdInput ? resumeIdInput.value.trim() : "";
    if (!sid) return;
    if (ctx.ws && ctx.connected) {
      ctx.ws.send(JSON.stringify({ type: "resume_session", cliSessionId: sid }));
    }
    closeResumeModal();
    closeSidebar();
  }
  ...
  if (resumeIdInput) {
    resumeIdInput.addEventListener("input", function () {
      if (resumeIdBtn) resumeIdBtn.disabled = !this.value.trim();
    });
    resumeIdInput.addEventListener("keydown", function (e) {
      if (e.key === "Enter") {
        e.preventDefault();
        importSessionById();
      }
    ...
    resumeIdInput.addEventListener("keyup", function (e) { e.stopPropagation(); });
    resumeIdInput.addEventListener("keypress", function (e) { e.stopPropagation(); });
  }

  if (resumeIdBtn) {
    resumeIdBtn.addEventListener("click", importSessionById);
  }
  ...
  // --- Mobile swipe gesture: swipe left to open projects ---
  var swipeStartX = 0;
  var swipeStartY = 0;
  ...
  document.addEventListener("touchend", function (e) {
    if (!swipeTracking || window.innerWidth > 768) return;
    swipeTracking = false;
    var touch = e.changedTouches[0];
    var deltaX = touch.clientX - swipeStartX;
    var deltaY = touch.clientY - swipeStartY;
    if (deltaX < -60 && Math.abs(deltaY) < Math.abs(deltaX) * 0.7) {
      openMobileSheet("projects");
    }
  }, { passive: true });
}
...
export function populateCliSessionList(sessions) {
  ...
  for (var i = 0; i < sessions.length; i++) {
    ...
    if (s.sessionId) {
      ...
      sid.className = "badge cli-session-id";
      sid.textContent = s.sessionId.substring(0, 12) + "...";
