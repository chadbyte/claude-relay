...
var SKIN_KEY = "clay-skin";        // legacy single key (migrated below)
var DARK_SKIN_KEY = "clay-skin-dark";
var LIGHT_SKIN_KEY = "clay-skin-light";
...
export function applyTheme(themeId, fromPicker) {
  ...
  // When picked from skin selector, save as skin preference for this variant and sync mode
  if (fromPicker) {
    try {
      var variantKey = isLight ? LIGHT_SKIN_KEY : DARK_SKIN_KEY;
      localStorage.setItem(variantKey, themeId);
...
// Looks up the saved skin for the target variant
function themeIdForMode(mode) {
  var key = mode === "light" ? LIGHT_SKIN_KEY : DARK_SKIN_KEY;
  var skin = null;
  try { skin = localStorage.getItem(key); } catch (e) {}

  // Migrate legacy single SKIN_KEY if per-variant keys are empty
  if (!skin) {
    var legacy = null;
    try { legacy = localStorage.getItem(SKIN_KEY); } catch (e) {}
    if (legacy) {
      var legacyTheme = getTheme(legacy);
      if (legacyTheme && legacyTheme.variant === mode) return legacy;
    }
  }

  if (skin && getTheme(skin)) return skin;
...
function updateToggleIcon() {
  ...
  var checkbox2 = document.getElementById("user-island-theme-check");
  if (checkbox2) checkbox2.checked = isLight;
}
...
export function initTheme() {
  ...
  var toggleCheck2 = document.getElementById("user-island-theme-check");
  if (toggleCheck2) {
    toggleCheck2.addEventListener("change", function () {
      toggleDarkMode();
    });
  }
