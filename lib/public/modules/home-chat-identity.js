...
function currentUser() {
  ...
  for (var i = 0; i < users.length; i++) {
    if (!users[i] || users[i].id !== myUserId) continue;
    return users[i].profile ? Object.assign({}, users[i], users[i].profile) : users[i];
  }
  return { id: myUserId || "you", avatarSeed: myUserId || "you" };
}
...
export function createHomeOrdinaryBubble(message, mate, mateName, time) {
  ...
  var row = isUser ? createUserBubble({
    text: message.text || "",
    name: name,
    avatarUrl: userAvatarUrl(currentUser(), 34),
    time: time || "",
