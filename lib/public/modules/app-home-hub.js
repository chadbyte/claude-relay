...
function syncHomeOverlayControls() {
  var reveal = document.getElementById("home-project-reveal");
  var workspace = document.getElementById("home-workspace");
  var dockActions = document.querySelector("#home-tool-workbench .home-dock-actions");
  if (!reveal || !workspace || !dockActions) return;
  var docked = store.get('dockOpen') === true;
  var target = docked ? dockActions : workspace;
  if (reveal.parentNode !== target) {
    if (docked) target.insertBefore(reveal, target.firstChild);
    else target.appendChild(reveal);
  }
  reveal.classList.toggle("is-docked", docked);
  reveal.classList.toggle("hidden", !store.get('currentSlug'));
}
...
function syncMateContextControls(mate) {
  ...
  var controls = [
    ["home-sidebar-new", "Start a new chat with "],
...
export function initHomeHub() {
  ...
  document.getElementById("home-project-reveal").addEventListener("click", minimizeHomeHub);
  ...
  store.subscribe(function (state, prev) {
    ...
    if (state.dockOpen !== prev.dockOpen || state.currentSlug !== prev.currentSlug) syncHomeOverlayControls();
