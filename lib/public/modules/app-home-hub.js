...
export function renderHomeHubMates() {
  ...
  if (!container) return;
  container.innerHTML = "";
  // Home is where mates live, so the list is complete here: Clay first as
  // everyone's default mate, then the rest. Only archived mates (e.g. Ally
  // after Clay took over) stay hidden.
  ...
  // Creating a mate now lives where mates live.
...
// The C button is the only entry point to mates now, so unread DMs and
// active mentions have to be visible on it.
