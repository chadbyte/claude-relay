...
export function renderHomeMateSwitcher() {
  ...
  if (homeHubVisible && store.get('homeSurfaceLoaded') && !activeMate) {
    var preferredMateId = store.get('homePreferredMateId');
    for (var j = 0; j < visibleMates.length; j++) {
      if (visibleMates[j].id === preferredMateId) {
        openHomeChat(visibleMates[j].id);
        return;
      }
    }
    for (var k = 0; k < visibleMates.length; k++) {
      if (visibleMates[k].builtinKey === "clay") {
        openHomeChat(visibleMates[k].id);
        return;
      }
    }
  }
