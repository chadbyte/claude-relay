...
function syncMateContextControls(mate) {
  ...
  var defaultModel = mate ? (mate.model ? (mate.vendor || "claude") + " · " + mate.model : "Choose model") : "Loading model…";
  var controls = [
    ["home-sidebar-new", "Start a new conversation with "],
    ["home-sidebar-memory", "Open memory for "],
    ["home-sidebar-knowledge", "Open knowledge for "],
    ["home-sidebar-model", "Choose a model for "],
    ["home-sidebar-settings", "Open Mate settings for "],
  ...
  for (var i = 0; i < controls.length; i++) {
    ...
    if (controls[i][0] === "home-sidebar-model" && mate) {
      control.setAttribute("aria-label", "Choose a model for " + name + ". Used for new conversations: " + defaultModel);
    } else {
      control.setAttribute("aria-label", mate ? controls[i][1] + name : controls[i][1] + "the current Mate");
    }
  }
  var modelValue = document.getElementById("home-sidebar-model-value");
  if (modelValue) {
    modelValue.textContent = defaultModel;
    modelValue.classList.toggle("hidden", !mate && !store.get('homeChatMateId'));
    modelValue.title = "";
  }
}
...
export function renderHomeMateSwitcher() {
  ...
  var focusedRow = null;
  if (focusedMateId) {
    focusedRow = findMateRow(list, focusedMateId);
