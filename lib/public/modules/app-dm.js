...
import { updateDmBadge, setCurrentDmUser, renderUserStrip, closeDmUserPicker } from './sidebar-mates.js';
...
export function handleMateCreatedInApp(mate, msg) {
  if (!mate) return;
  var s = store.snap();
  s.cachedMatesList.push(mate);
  if (msg && msg.availableBuiltins) store.set({ cachedAvailableBuiltins: msg.availableBuiltins });
  if (msg && msg.dmFavorites) store.set({ cachedDmFavorites: msg.dmFavorites });
  var s2 = store.snap();
  renderUserStrip(s2.cachedAllUsers, s2.cachedOnlineIds, s2.myUserId, s2.cachedDmFavorites, s2.cachedDmConversations, s2.dmUnread, s2.dmRemovedUsers, s2.cachedMatesList);
