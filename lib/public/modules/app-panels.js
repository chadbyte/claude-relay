...
export function initPanels() {
  ...
  function updateVendorToggle() {
    ...
    // Show/hide toggle (hide if only one vendor or session is active)
    if (available.length < 2 && !available.some(function(v) { return !vendorBtns[v]; })) {
      // Still show toggle for feature discovery even with 1 vendor
    }
    ...
    for (var i = 0; i < vendors.length; i++) {
      ...
      if (!btn) continue;
      var isAvailable = available.indexOf(v) !== -1;
  ...
  function onVendorClick(vendor) {
    ...
    var available = store.get('availableVendors') || [];
    if (available.indexOf(vendor) === -1) {
      var cmds = { claude: "claude login", codex: "codex login" };
      showToast("Run `" + (cmds[vendor] || vendor + " login") + "` in terminal to enable " + (VENDOR_NAMES[vendor] || vendor), 4000);
      return;
    }
    store.set({ currentVendor: vendor });
    // Update models for the new vendor (server will send when session starts)
    var ws = getWs();
