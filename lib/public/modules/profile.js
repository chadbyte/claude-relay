...
function applyToIsland() {
  ...
  if (existingImg) {
    ...
    img.onerror = function () {
      ...
      if (existingLetter) {
        existingLetter.style.display = '';
        existingLetter.textContent = (displayName || '?').charAt(0).toUpperCase();
      }
    };
...
function showPopover() {
  ...
  for (var j = 0; j < AVATAR_STYLES.length; j++) {
    ...
    html += '<img src="' + avatarUrl(st.id, seed, 40) + '" alt="' + st.name + '" loading="eager" crossorigin="anonymous" onerror="this.style.opacity=\'0.3\'">';
    html += '</button>';
  }
  html += '</div>';
  html += '</div>';
  ...
  try { isFullWidth = localStorage.getItem('clay-full-width') === 'true'; } catch (e) {}
  html += '<div class="profile-field">';
  html += '<label class="profile-field-label">Chat Layout</label>';
  html += '<label class="profile-toggle-row">';
  html += '<span class="profile-toggle-label">Full-width messages</span>';
  html += '<input type="checkbox" id="profile-fullwidth-toggle" class="profile-toggle-check"' + (isFullWidth ? ' checked' : '') + '>';
  ...
  // Full-width toggle
  var fullWidthToggle = popoverEl.querySelector('#profile-fullwidth-toggle');
  if (fullWidthToggle) {
    fullWidthToggle.addEventListener('change', function () {
      var enabled = this.checked;
      try { localStorage.setItem('clay-full-width', enabled ? 'true' : 'false'); } catch (e) {}
      document.documentElement.classList.toggle('full-width-chat', enabled);
    });
  }
