...
var UI_NODE_TYPES = [
  ...
  "input", "textarea", "select", "checkbox", "button", "badge", "divider", "empty-state",
  "board", "board-card",
];
var UI_ICONS = [
...
function validateUiNode(node, location, context, depth) {
  ...
  var props = node.props || {};
  if (["input", "textarea", "select", "checkbox"].indexOf(node.type) !== -1 && (!node.bind || !node.action)) throw new Error("Interactive " + node.type + " at " + location + " requires bind and action.");
  if (["input", "textarea", "select", "checkbox"].indexOf(node.type) !== -1 && !props.label && !node.id) throw new Error("Interactive " + node.type + " at " + location + " requires a label or ID.");
