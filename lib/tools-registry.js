...
var SEED_MARKER = ".capsules-v5";
var PREVIOUS_SEED_MARKER = ".capsules-v4";
var V3_SEED_MARKER = ".capsules-v3";
...
var TRANSLATOR_MODEL_SELECT_HASHES = {
  ui: ["385761364254bcbd92873a2bca39c4ab3ff596061f7a29da53b3f2850b8c93c9", "49541e5abe5dac445193da9872cc486b4e3f0c6216c0ff7d31bca50ad24bb16c"],
  logic: "97293d16c10156c04054d1a04dda1fbb0492b8e15510235ff00f8a58965691cc",
};
var ADVANCED_UI_HASHES = {
  translator: { ui: "599e45908b19393fea157c54da05139b650b6cd753e895946ae9e37a87bc9413", logic: "a6666fcc4c4a7e6e26e18449ca55b3bbf1442a3abadb531b605d8e6ddb0e4de0" },
  scratchpad: { ui: "abf73707613106e503e65b13eda0569f3f2e3c74429656c7ae7eed9dce0d91e2", logic: "0eed00e2d172a21223156ba52931095ed421ed6a2fc7a31db4ebba18d82a7d47" },
};
...
function upgradeUntouchedTranslatorModelSelect(destination) {
  ...
  try {
    ...
    if (!installedManifest.example || !manifestsMatchForUiUpgrade(installedManifest, shippedManifest)) return false;
    if (TRANSLATOR_MODEL_SELECT_HASHES.ui.indexOf(fileHash(path.join(destination, "ui.json"))) === -1) return false;
    if (fileHash(path.join(destination, "logic.js")) !== TRANSLATOR_MODEL_SELECT_HASHES.logic) return false;
...
function upgradeUntouchedAdvancedUi(destination, name) {
  var expected = ADVANCED_UI_HASHES[name];
  ...
  try {
    ...
    var shippedManifest = JSON.parse(fs.readFileSync(path.join(source, "manifest.json"), "utf8"));
    if (!installedManifest.example || !manifestsMatchForUiUpgrade(installedManifest, shippedManifest)) return false;
    if (fileHash(path.join(destination, "ui.json")) !== expected.ui) return false;
    if (fileHash(path.join(destination, "logic.js")) !== expected.logic) return false;
    fs.copyFileSync(path.join(source, "ui.json"), path.join(destination, "ui.json"));
    fs.copyFileSync(path.join(source, "logic.js"), path.join(destination, "logic.js"));
    return true;
  } catch (e) {
...
function seedBuiltInCapsules(ctx) {
  ...
  var upgradingV4 = fs.existsSync(path.join(root, PREVIOUS_SEED_MARKER));
  var upgradingV3 = !upgradingV4 && fs.existsSync(path.join(root, V3_SEED_MARKER));
  var upgradingV2 = !upgradingV4 && !upgradingV3 && fs.existsSync(path.join(root, V2_SEED_MARKER));
  var upgradingV1 = !upgradingV4 && !upgradingV3 && !upgradingV2 && fs.existsSync(path.join(root, LEGACY_SEED_MARKER));
  ...
  for (var i = 0; i < entries.length; i++) {
    ...
    if (upgradingV4) {
      if (fs.existsSync(destination) && upgradeUntouchedAdvancedUi(destination, entries[i].name)) seeded.push(entries[i].name + ":advanced-ui");
      continue;
    }
    if (upgradingV3) {
  ...
  if (!fs.existsSync(path.join(root, PREVIOUS_SEED_MARKER))) fs.writeFileSync(path.join(root, PREVIOUS_SEED_MARKER), seeded.join("\n") + "\n", "utf8");
}
