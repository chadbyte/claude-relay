...
function resolveBoardRoot(ctx) {
  return path.join(toolsRegistry.resolveToolsRoot(ctx), "board");
}

function resolveLegacyBoardRoot(ctx) {
...
function createBoardManager(ctx) {
  ...
  function getDatastore() {
    if (!datastoresByRoot[root]) {
      ...
      var dataFile = path.join(root, "data.db");
      var legacyFile = path.join(resolveLegacyBoardRoot(ctx), "board.db");
