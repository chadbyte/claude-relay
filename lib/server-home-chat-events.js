...
function transformEvent(event, mateId, session, requestId, stableSessionId) {
  ...
  if (session && session.homeClayEntryMode === "search") {
    if (event.type === "thinking_start") return Object.assign({ type: "home_clay_activity", phase: "thinking", step: session._homeClayActivityStep || 0 }, metadata);
    if (event.type === "tool_start") {
      session._homeClayActivityStep = (session._homeClayActivityStep || 0) + 1;
      return Object.assign({ type: "home_clay_activity", phase: "searching", step: session._homeClayActivityStep }, metadata);
    }
    if (event.type === "tool_result") return Object.assign({ type: "home_clay_activity", phase: event.is_error ? "reconsidering" : "reviewing", step: session._homeClayActivityStep || 1 }, metadata);
  }
