...
function attachMessageProcessor(ctx) {
  ...
  function processSDKMessage(session, parsed) {
    ...
    } else if (parsed.yokeType === "interrupted") {
      // Turn was interrupted (abort/stop). Send feedback and done.
      var vendorNames = { claude: "Claude", codex: "Codex", gemini: "Gemini" };
      var interruptVendor = vendorNames[session.vendor] || "Agent";
      session.isProcessing = false;
      onProcessingChanged();
      sendAndRecord(session, { type: "info", text: "Interrupted \u00b7 What should " + interruptVendor + " do instead?" });
