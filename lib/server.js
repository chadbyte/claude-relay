...
var { pinPageHtml, setupPageHtml, adminSetupPageHtml, multiUserLoginPageHtml, invitePageHtml, noProjectsPageHtml } = require("./pages");
...
function createServer(opts) {
  ...
  var appHandler = function (req, res) {
      ...
      req.on("end", function () {
        try {
          ...
          if (!data.username || data.username.length < 1 || data.username.length > 30) {
            res.writeHead(400, { "Content-Type": "application/json" });
            res.end('{"error":"Username must be 1-30 characters"}');
            return;
          }
          if (!/^[a-zA-Z0-9_-]+$/.test(data.username)) {
            res.writeHead(400, { "Content-Type": "application/json" });
            res.end('{"error":"Username can only contain letters, numbers, hyphens, and underscores"}');
            return;
          }
    ...
    if (req.method === "GET" && fullUrl.indexOf("/invite/") === 0) {
      ...
      res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
      res.end(invitePageHtml(inviteCode));
      return;
    }
    ...
    if (req.method === "GET" && /^\/api\/admin\/projects\/[a-z0-9_-]+\/access$/.test(fullUrl)) {
      ...
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end('{"visibility":"public","allowedUsers":[]}');
      return;
    }
    ...
    if (req.method === "GET" && fullUrl === "/api/me") {
      ...
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ multiUser: true, user: { id: mu.id, username: mu.username, displayName: mu.displayName, role: mu.role } }));
      return;
    }
