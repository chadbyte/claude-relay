...
function createServer(opts) {
  ...
  function getAuthPage() {
    ...
    if (smtp.isSmtpConfigured()) return smtpLoginPage;
    return loginPage;
  }
  ...
  var appHandler = function (req, res) {
    ...
    if (req.method === "POST" && fullUrl === "/auth/register") {
      ...
      req.on("end", function () {
        try {
          ...
          var result;
          if (smtp.isSmtpConfigured() && !data.pin) {
    ...
    if (req.method === "GET" && fullUrl.indexOf("/invite/") === 0) {
      ...
      res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
      res.end(smtp.isSmtpConfigured() ? smtpInvitePageHtml(inviteCode) : invitePageHtml(inviteCode));
      return;
    }
    ...
    if (req.method === "GET" && fullUrl === "/api/admin/smtp") {
      ...
      if (cfg) {
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ smtp: { host: cfg.host, port: cfg.port, secure: cfg.secure, user: cfg.user, pass: "••••••••", from: cfg.from } }));
      } else {
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end('{"smtp":null}');
      }
    ...
    if (req.method === "GET" && fullUrl === "/api/me") {
      ...
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ multiUser: true, smtpEnabled: smtp.isSmtpConfigured(), user: { id: mu.id, username: mu.username, email: mu.email || null, displayName: mu.displayName, role: mu.role } }));
      return;
    }
