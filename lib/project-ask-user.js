...
function attachAskUser(ctx) {
  ...
  function normalizeFirstDebateQuestion(session, input) {
    if (!isFirstDebateQuestion(session)) return input;
    var source = input && Array.isArray(input.questions) && input.questions[0] ? input.questions[0] : {};
    return { questions: [{ header: source.header || "", question: source.question || "", multiSelect: false, options: [] }] };
  }
  ...
  function getToolDefs(boundSession) {
    return askUserMcp.getToolDefs(function onAsk(input) {
      ...
      input = normalizeFirstDebateQuestion(boundSession, input);
