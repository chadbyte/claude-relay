...
// reads the ledger and appends attributed comments. That keeps the log an
// honest record of the work rather than a shared wiki.
...
// notify.
var NOTIFYING_OPS = ["create", "update", "link", "incorporate", "revert"];
...
function attachProjectLogs(ctx) {
  ...
  function listEntries(bound, query, category) {
    if (query) {
      var hits = bound.searchLogs({ query: query, kind: category, limit: LIST_LIMIT });
      return { entries: hits.results, categories: hits.categories || [] };
    }
    var listed = bound.listLogs({ kind: category, limit: LIST_LIMIT });
    return { entries: listed.entries, categories: listed.categories || [] };
  }
  ...
  function handleLogsMessage(ws, msg) {
    ...
    try {
      if (msg.type === "project_logs_list") {
        ...
        var listing = listEntries(bound, query, category);
        sendTo(ws, {
