...
function attachDebate(ctx) {
    ...
    ctx.sm.sessions.forEach(function (session) {
      ...
      if (session.debateState) {
        ...
        if (session.homeDebatePlanning === true && (phase === "live" || phase === "ending")) {
          ...
          var hasHomeEnd = session.history.some(function (event) { return event && event.type === "debate_ended"; });
          if (!hasHomeEnd) session.history.push({ type: "debate_ended", topic: session.debateState.topic || session.title || "Debate", rounds: session.debateState.round || 1, reason: "interrupted", _ts: Date.now() });
          ctx.sm.saveSessionFile(session);
          return;
        }
  ...
  function handleHomeControl(ws, msg, session) {
    if (!session || session.homeDebatePlanning !== true || !session._debate) return false;
    if (msg.action === "hand_raise") handleDebateHandRaise(ws, session);
    else if (msg.action === "comment") handleDebateComment(ws, msg, session);
    else if (msg.action === "stop") handleDebateStop(ws, session);
