...
function attachDebate(ctx) {
  ...
  function startDebateLive(session, targetWs) {
    ...
    var debateSession = ctx.sm.createSession(liveOpts, targetWs || null);
  ...
  function handleMcpDebateApproval(session, briefData, mateId, ws) {
    ...
    if (homeWs) Object.defineProperty(debate, "homeClient", { value: homeWs, writable: true, configurable: true, enumerable: false });
    if (homeWs) homeWs._clayHomePreviousSessionId = homeWs._clayActiveSession || null;
    if (homeWs && typeof ctx.registerHomeDebateClient === "function") ctx.registerHomeDebateClient(homeWs);
    if (homeWs) homeWs._clayHomeDebateSlug = ctx.slug;
