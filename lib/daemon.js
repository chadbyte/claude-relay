...
var relay = createServer({
  ...
  onSetProjectMcpServers: function (slug, servers) {
    console.log("[daemon] setMcpServers slug=" + slug + " servers=" + JSON.stringify(servers) + " projects=" + config.projects.map(function(p){return p.slug}).join(","));
    for (var i = 0; i < config.projects.length; i++) {
      if (config.projects[i].slug === slug) {
        ...
        saveConfig(config);
        console.log("[daemon] saved. verify: " + JSON.stringify(config.projects[i].enabledMcpServers));
        return;
      }
    }
    console.log("[daemon] slug not found!");
  },
