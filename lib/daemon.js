...
// Register named agents from ~/.claude/agents/*.md (chat: true opt-in).
// Identity is loaded via per-agent CLAUDE.md materialized into the working
// dir; Claude CLI's normal project-context loader injects it as the system
// prompt. See chadbyte/clay#332. Fork-only feature.
try {
  var chatAgents = agents.discoverChatAgents();
  for (var ai = 0; ai < chatAgents.length; ai++) {
    var ag = chatAgents[ai];
    var agDir = agents.getAgentDir(ag.slug);
    try { agents.materializeAgentClaudeMd(ag); }
    catch (e) {
      console.error("[daemon] Failed to materialize CLAUDE.md for agent " + ag.slug + ":", e.message);
      continue;
    }
    var agSlug = agents.projectSlugFor(ag);
    var agTitle = ag.name;
    // Session cwd is the agent's home_project when declared (so Sentinel
    // can attribute the JSONL to a workspace project). Identity comes
    // from the agent body, injected explicitly via systemPrompt — see
    // sdk-bridge.js. Falls back to the per-agent ~/.clay/agents/<slug>/
    // dir for back-compat when home_project is absent.
    var agCwd = ag.homeProject || agDir;
    console.log("[daemon] Adding agent project:", agSlug, "→", agCwd,
      ag.homeProject ? "(home_project)" : "(default agent dir)");
    relay.addProject(agCwd, agSlug, agTitle, null, null, null, {
      isAgent: true,
      agentName: ag.name,
      agentDescription: ag.description,
      agentModel: ag.model,
      agentSourcePath: ag.sourcePath,
      agentBody: ag.body,
      agentHomeDir: agDir,
    });
  }
} catch (e) {
  console.error("[daemon] Agent registration failed:", e.message);
}
