...
// --- Absorb a legacy single-user deploy into the multi-user model (one-time,
// idempotent, best-effort). Runs before projects/mates/sessions load so the
// multi-user flip and on-disk backfill are in effect for the rest of boot. ---
try {
  require("./migrate-single-user").migrateSingleUserToMulti();
  config = loadConfig(); // refresh in-memory config to pick up the migration marker
} catch (e) {
  console.error("[daemon] single-user migration threw (continuing):", e && e.message);
}
