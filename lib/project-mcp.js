...
function attachMcp(ctx) {
  ...
  function handleToggleServer(ws, msg) {
    ...
    console.log("[mcp-bridge] toggle: " + name + "=" + enabled + " list=" + JSON.stringify(list) + " slug=" + slug);
    setEnabledMcpServers(list);
    console.log("[mcp-bridge] after set, get returns: " + JSON.stringify(getEnabledMcpServers()));
    rebuildProxyServers();
    broadcastMcpState();
  }
  ...
  function rebuildProxyServers() {
    _proxyServers = {};
    var availNames = Object.keys(_availableServers);
    var enabledDbg = getEnabledMcpServers() || [];
    console.log("[mcp-bridge] rebuild: available=" + availNames.join(",") + " enabled=" + enabledDbg.join(","));
    for (var di = 0; di < availNames.length; di++) {
      var dbgS = _availableServers[availNames[di]];
      console.log("[mcp-bridge]   " + availNames[di] + ": enabled=" + dbgS.enabled + " tools=" + (dbgS.tools || []).length);
    }
