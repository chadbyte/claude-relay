...
var LOGS_CONTRACT =
  "Project Logs are this project's durable record of decisions and work, written the way a good Git history reads. " +
  ...
  "Write an entry when the project gains something worth knowing weeks from now: a decision and the reasoning behind it, a security finding, an investigation's outcome, an incident and its resolution, an operational runbook, a durable reference, or real progress. " +
  "Every entry needs a concise meaningful title, a one or two sentence summary explaining what was decided or done and why, and a category. Set priority when an entry genuinely outranks routine work. " +
...
// treating them as one. A note is an alert that should disappear once the thing
// it is shouting about is handled; a log entry is the permanent record of what
// happened. Conflating them either fills the board with resolved history or
// loses the history when the board is cleared.
//
...
var ATTENTION_CONTRACT =
  ...
  "A Sticky Note is the transient attention layer: an unresolved, actionable commitment or defect that stays visible only while it still needs action, and is removed once it no longer does. " +
  "A Project Log is the durable project-scoped ledger: discovery, evidence, impact, decision, remediation, and outcome stay permanently, versioned, long after the note is gone. " +
  ...
  "A log entry may mention that an attention note exists, but must never depend on a note id for its identity, because notes are removed and ids do not survive. " +
  ...
  "When the defect is fixed, revise that same entry with the remediation, how it was verified, and the outcome as a new canonical revision, and only then remove the Sticky Note. The note goes; the entry stays. " +
...
function projectTools(bound) {
  return [
    ...
    {
      name: "create_log",
      description: LOGS_CONTRACT + " " + LEARNING_CONTRACT + " Create a log entry in this project. Apply the test first: will someone need this fact weeks from now, and is it not already recorded? If it supersedes an existing entry, revise that entry instead of adding a new one.",
      inputSchema: buildShape({
    ...
    {
      name: "update_log",
      description: LOGS_CONTRACT + " " + LEARNING_CONTRACT + " Revise an existing log entry, including when a later decision supersedes it, or when new learning refines an existing learning entry. The previous revision, its title, and its summary are all retained in the entry history.",
      inputSchema: buildShape({
        ref: { type: "string", description: REF_DESCRIPTION },
        kind: { type: "string", description: CATEGORY_DESCRIPTION },
        priority: { type: "string", enum: logsSchema.PRIORITIES, description: PRIORITY_DESCRIPTION },
