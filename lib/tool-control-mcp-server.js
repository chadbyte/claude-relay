...
var INSTALL_DESCRIPTION = [
  ...
  "uiTree nodes are JSON {type,id?,props?,children?,bind?,action?}. Vocabulary: stack, row, text, heading, list, table, card, input, textarea, select, checkbox, button, badge, divider, empty-state. bind is a dot-path into state; interactive action names a logic action. A list binds an array and renders its child template per item; use $item.field in bind and props.args values.",
  "logicSource must define: var tool = { initialState: {...}, actions: { name: function (state, args, api) { ... return newState; } } }. Async actions may use api.storage.list/get/put/delete/query, api.llm.complete({system?,prompt,model?}) when manifest permissions includes llm, and api.callerId. model is only fast|standard|deep. No DOM, imports, network globals, or vendor model names. Return the complete next state.",
].join(" ");
