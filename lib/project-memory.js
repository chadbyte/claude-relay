...
function attachMemory(ctx) {
  ...
  var getWorkspaceMemorySessions = ctx.getWorkspaceMemorySessions || function () { return []; };
  var getOwnedCurrentSessions = ctx.getOwnedCurrentSessions || function () { return []; };
  ...
  function loadMateDigests(mateCtx, mateId, query) {
    ...
    if (hasGlobalSearch) {
      ...
      try {
        ...
        // Own sessions
        getOwnedCurrentSessions().forEach(function (s) {
        ...
        var crossForTimeline = getWorkspaceMemorySessions();
    ...
    if (query && (hasGlobalSearch || allLines.length > 5)) {
      try {
        ...
        var knowledgeFiles = [];
        if (hasGlobalSearch) {
          var crossSessions = getWorkspaceMemorySessions();
  ...
  function handleMemoryList(ws) {
    ...
    sendTo(ws, { type: "memory_list", entries: state.entries, summary: state.summary });
  }
  ...
  return {
    ...
    readMemoryState: readMemoryState,
