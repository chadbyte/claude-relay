...
function createProjectContext(opts) {
  ...
  var loopRegistry = createLoopRegistry({
    ...
    onTrigger: function (record) {
      // Schedule records are calendar-only — never execute as loops
      if (record.source === "schedule") {
        console.log("[loop-registry] Schedule event triggered (calendar-only): " + record.name);
        send({ type: "schedule_event_fired", record: record });
        return;
      }
