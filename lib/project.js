...
function createProjectContext(opts) {
  ...
  var mcpServers = (function () {
    ...
    // Ask-user MCP server (mates only; static instance fails closed until bound per query)
    if (isMate) {
      try {
        var askUserMcpConfig = _askUser.createMcpServer(adapter, null);
  ...
  function getLocalMcpServers(forSession) {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      if (name === "clay-ask-user" && forSession) {
        try {
          var boundAskUser = _askUser.createMcpServer(adapter, forSession);
          if (boundAskUser) { filtered[name] = boundAskUser; hasAny = true; }
        } catch (e) {
          console.error("[project] Failed to bind ask-user MCP server:", e.message);
        }
        continue;
      }
  ...
  sdk = createSDKBridge({
    ...
    getSessionToolDefs: function (session) {
      return _sessionPair.getToolDefs(session)
        ...
        .concat(session && session.debateSetupMode === true ? _askUser.getToolDefs(session) : []);
    },
  });
  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type === "home_debate_question_response" || msg.type === "home_debate_control") {
      if (typeof opts.onDmMessage === "function") opts.onDmMessage(ws, msg);
      return;
    }
    if (msg.type === "dm_open" || msg.type === "dm_send" || msg.type === "dm_list" || msg.type === "dm_typing" || msg.type === "dm_add_favorite" || msg.type === "dm_remove_favorite" || msg.type === "mate_create" || msg.type === "mate_list" || msg.type === "mate_delete" || msg.type === "mate_update" || msg.type === "mate_readd_builtin" || msg.type === "mate_list_available_builtins" || msg.type === "email_accounts_list" || msg.type === "email_account_add" || msg.type === "email_account_remove" || msg.type === "email_account_test" || msg.type === "home_mate_present" || msg.type === "home_mate_open" || msg.type === "home_mate_sessions_list" || msg.type === "home_mate_session_open" || msg.type === "home_mate_send" || msg.type === "home_mate_new_session" || msg.type === "home_mate_debate_plan" || msg.type === "home_debate_proposal_response" || msg.type === "home_mate_close" || msg.type === "home_mate_memory_list" || msg.type === "home_mate_knowledge_list" || msg.type === "home_mate_models_get" || msg.type === "home_mate_model_set" || msg.type === "home_dock_get" || msg.type === "home_dock_set" || msg.type === "home_surface_get" || msg.type === "home_surface_set" || msg.type === "board_list" || msg.type === "board_card_create" || msg.type === "board_card_update" || msg.type === "board_card_move" || msg.type === "board_card_delete" || msg.type === "board_done_confirm" || msg.type === "tools_list" || msg.type === "tool_get" || msg.type === "tool_install" || msg.type === "tool_remove" || msg.type === "tool_storage_op" || msg.type === "tool_llm_op" || msg.type === "tool_llm_config_get" || msg.type === "tool_control_response" || msg.type === "tool_source_get" || msg.type === "tool_mate_access_set") {
  ...
  function getMcpBridgeHandler(sessionId, sessionOnly) {
    ...
    return {
      listTools: function () {
        ...
        if (localMcp) {
          ...
          for (var i = 0; i < inAppNames.length; i++) {
            ...
            if (inAppNames[i] === "clay-debate") continue;
      ...
      callTool: function (serverName, toolName, args) {
        ...
        if (serverName === "clay-sessions" || serverName === "clay-notes" || serverName === "clay-handoff" || serverName === "clay-documents" || serverName === "clay-debate") {
