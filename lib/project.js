...
function createProjectContext(opts) {
  ...
  function createToolControlMcpServer() {
    ...
    var userId = projectOwnerId || "default";
    var mateId = path.basename(cwd);
    var handlers = {
      list: function () {
        var manifests = listControllableTools(userId) || [];
        var tools = [];
      ...
      snapshot: function (toolId) {
        return controlToolForMate(userId, mateId, toolId, "snapshot", {});
      },
      act: function (toolId, actionId, args) {
        return controlToolForMate(userId, mateId, toolId, "act", { actionId: actionId, args: args });
      },
      set: function (toolId, controlId, value) {
        return controlToolForMate(userId, mateId, toolId, "set", { controlId: controlId, value: value });
      },
      install: function (input) {
        return installToolForMate(userId, input);
      },
      uninstall: function (toolId) {
        return removeToolForMate(userId, toolId);
      },
    };
