...
function attachEmail(ctx) {
  ...
  // Create MCP server dependencies
  function createMcpDeps(userId) {
    return {
      getAccountForTool: function (email) {
        return emailAccounts.getAccountByEmailDecrypted(userId, email);
      },
      getCheckedAccounts: function () {
        return getAllCheckedAccounts(userId);
      },
