...
function createCodexQueryHandle(appServer, queryOpts) {
  ...
  var handle = {
    ...
    abort: function() {
      state.aborted = true;
      // Send turn/interrupt as a request (not notification) to app-server
      // Don't endIterator yet - wait for turn/completed with status "interrupted"
      // so sdk-bridge processes the completion and resets UI properly
      if (state.threadId && appServer.started) {
        appServer.send("turn/interrupt", { threadId: state.threadId }, 5000).then(function() {
          // Server acknowledged interrupt. turn/completed will arrive via event stream.
        }).catch(function(e) {
          // If interrupt fails (turn already done), force end
          if (turnResolve) {
            var resolve = turnResolve;
            turnResolve = null;
            resolve();
