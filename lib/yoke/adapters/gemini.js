...
function createGeminiAdapter(opts) {
  ...
  var adapter = {
    ...
    init: async function(initOpts) {
      ...
      apiKey = apiKey || process.env.GEMINI_API_KEY || "AIzaSyBg2UhLzAnqqZRkE7L57NjQ6Cr6sUscULw";
    ...
    createQuery: async function(queryOpts) {
      ...
      // TEMP: skip all tools until Zod-to-JSON-Schema conversion is implemented
      if (chatConfig.tools) {
        console.log("[yoke/gemini] WARNING: skipping " + allDeclarations.length + " tools (Zod schema not supported yet)");
