...
function createSDKBridge(opts) {
  ...
  async function processQueryStream(session) {
    ...
    console.log("[sdk-bridge] processQueryStream: starting for-await loop, vendor=" + adapter.vendor);
    try {
  ...
  async function startQuery(session, text, images, linuxUser) {
    console.log("[sdk-bridge] startQuery: vendor=" + adapter.vendor + " session=" + session.localId + " text=" + (text || "").substring(0, 50));
    ...
    var queryOpts = {
      cwd: cwd,
      model: ls.model || sm.currentModel || undefined,
    ...
    var handle;
    console.log("[sdk-bridge] calling adapter.createQuery...");
    try {
      handle = await adapter.createQuery(queryOpts);
  ...
  async function warmup(linuxUser) {
    console.log("[sdk-bridge] warmup: vendor=" + adapter.vendor + " linuxUser=" + (linuxUser || "none"));
    try {
      ...
      send({ type: "model_info", model: sm.currentModel || "", models: sm.availableModels || [], vendor: adapter.vendor });
    } catch (e) {
  ...
  async function setModel(session, model) {
    if (!session.queryInstance) {
      ...
      send({ type: "model_info", model: model, models: sm.availableModels || [], vendor: adapter.vendor });
      send({ type: "config_state", model: sm.currentModel, mode: sm.currentPermissionMode || "default", effort: sm.currentEffort || "medium", betas: sm.currentBetas || [] });
      return;
    }
    try {
