...
function createSDKBridge(opts) {
        ...
        if (msg && msg.type === "_worker_meta") {
          ...
          switch (msg.subtype) {
            ...
            case "model_changed":
              ...
              send({ type: "model_info", model: metaData.model, models: sm.availableModels || [] });
  ...
  async function startQuery(session, text, images, linuxUser) {
    ...
    if (sm.currentThinking === "disabled") {
      ...
      claudeOpts.thinking = { type: "enabled", budgetTokens: sm.currentThinkingBudget };
    }
    ...
    var queryOpts = {
      cwd: cwd,
      model: sm.currentModel || undefined,
      effort: sm.currentEffort || undefined,
