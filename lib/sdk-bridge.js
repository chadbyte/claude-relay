...
function createSDKBridge(opts) {
    ...
    worker.onMessage(function(msg) {
      switch (msg.type) {
        ...
        case "query_error":
          ...
          var workerACEnabled = session.onQueryComplete || (typeof opts.getAutoContinueSetting === "function" && opts.getAutoContinueSetting());
  ...
  async function processQueryStream(session) {
    try {
      ...
      var acEnabled = session.onQueryComplete || (typeof opts.getAutoContinueSetting === "function" && opts.getAutoContinueSetting());
      if (session.rateLimitResetsAt && session.rateLimitResetsAt > Date.now()
