  ...
  async function processQueryStream(session) {
    ...
    try {
      ...
      if (session.isProcessing && session.taskStopRequested) {
        var vendorNames = { claude: "Claude", codex: "Codex", gemini: "Gemini" };
        var vName = vendorNames[session.vendor] || "the agent";
        session.isProcessing = false;
        onProcessingChanged();
        sendAndRecord(session, { type: "done", code: 0 });
        sendAndRecord(session, { type: "info", text: "\u25a0 Conversation interrupted - tell the model what to do differently." });
      ...
      if (session.isProcessing) {
        ...
        if (err.name === "AbortError" || (myAbortController && myAbortController.signal.aborted) || session.taskStopRequested) {
          if (!session.destroying) {
            var vendorNames2 = { claude: "Claude", codex: "Codex", gemini: "Gemini" };
            var vName2 = vendorNames2[session.vendor] || "the agent";
            sendAndRecord(session, { type: "done", code: 0 });
            sendAndRecord(session, { type: "info", text: "\u25a0 Conversation interrupted - tell the model what to do differently" + vName2 + " do instead?" });
          }
