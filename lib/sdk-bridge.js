...
function createSDKBridge(opts) {
  ...
  function checkToolWhitelist(toolName, input) {
    ...
    // Auto-approve the visible Driver/Split Worker pair tools, including the
    // Driver's answer to a Worker permission request. The pair is
    // user-constructed and both panes are on screen; every one of these tools
    // is structurally confined to the caller's own exact pair and re-checks
    // Driver eligibility, ownership and the live pair inside its handler, so
    // the whitelist grants no authority the handler would not already demand.
