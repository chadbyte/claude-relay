...
function createSDKBridge(opts) {
  ...
  function checkToolWhitelist(toolName, input) {
    ...
    // The Driver's answer to a Split Worker permission request must never
    // raise a prompt of its own: that prompt is the exact interaction being
    // routed away from the human, so asking about it would defeat the routing.
    // Exactly this one tool name, in both forms it can arrive under — the
    // dynamic/direct name (Codex canonicalizes only send_to_partner and
    // read_partner, so this one stays bare) and the clay-sessions MCP name.
    // No prefix is widened: the handler re-validates the live pair, the exact
    // Driver, the owner and the request id on every call, so the whitelist
    // grants no authority the handler would not already require.
    if (toolName === "respond_to_worker_permission"
        || toolName === "mcp__clay-sessions__respond_to_worker_permission") {
      return { behavior: "allow", updatedInput: input };
    }
