...
function createSDKBridge(opts) {
  ...
  var isAgent = opts.isAgent || (slug.indexOf("agent-") === 0);
  ...
  async function startQuery(session, text, images, linuxUser) {
    ...
    // Named-agent identity injection. When the session cwd is the agent's
    // home_project (e.g. /workspace/project-agents/), the YOKE instruction
    // scanner picks up the project's CLAUDE.md but NOT the agent's own
    // identity (which lives at ~/.clay/agents/<slug>/CLAUDE.md). Inject it
    // explicitly via systemPrompt so identity travels with the session
    // regardless of cwd. See lr-195d.
    if (isAgent) {
      var identity = agentBody;
      if (!identity && agentHomeDir) {
        try {
          var idPath = path.join(agentHomeDir, "CLAUDE.md");
          identity = fs.readFileSync(idPath, "utf8");
        } catch (e) { /* fall through */ }
      }
      if (identity && String(identity).trim()) {
        queryOpts.systemPrompt = String(identity).trim();
      }
    }
