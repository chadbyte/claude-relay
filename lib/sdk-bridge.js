...
function createSDKBridge(opts) {
  ...
  function handleCanUseTool(session, toolName, input, opts) {
    ...
    // Codex receives the session-bound AskUserQuestion fallback as a dynamic
    // tool. Calling it only posts the canonical question card; the answer is
    // still validated and correlated by the Home response route.
    if (session.debateSetupMode === true && toolName === "ask_user_questions") {
