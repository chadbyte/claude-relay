...
function createSDKBridge(opts) {
  ...
  var WORKER_SCRIPT = adapter.workerScriptPath || path.join(__dirname, "yoke", "adapters", "claude-worker.js");
  ...
  async function startQuery(session, text, images, linuxUser) {
    ...
    // Store handle on session for backward compat
    session.queryInstance = handle._rawQuery || handle;
    session.messageQueue = handle._messageQueue;

    // Build and push initial user message
    var content = [];
    if (images && images.length > 0) {
      for (var i = 0; i < images.length; i++) {
        content.push({
    ...
    handle._pushRaw({ type: "user", message: { role: "user", content: content } });
