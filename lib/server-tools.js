...
function attachTools(deps) {
  ...
  // from the server-resolved actor, never from caller-supplied text. The result
  // is state and nothing else: a Mate never sees Display.
  async function controlServerCapsule(userId, installed, actor, callerId, kind, payload) {
    ...
    if (kind === "act") return runtime.act(context, payload.actionId, payload.args || {});
  ...
  function handleMessage(ws, msg) {
    var messageTypes = ["tools_list", "tool_get", "tool_install", "tool_remove", "tool_storage_op", "tool_llm_op", "tool_llm_config_get", "tool_control_response", "tool_source_get", "tool_mate_access_set", "tool_server_control"];
