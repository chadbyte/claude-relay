...
function createProjectLogsStore(opts) {
  ...
  var scopeId = logsRoot.scopeIdForRoot(root);
  ...
  // The canonical record is reconstructed from its revision chain, and the
  // discussion is folded separately. Neither comments nor a clarify/decline
  // review touch the canonical state or the revision count.
  function fold(rootId, chain) {
    ...
    // A create record that does not carry the minimum a log entry needs is not
    // a log entry. Hand-edited files and torn writes that were later completed
    // can look structurally valid without being usable.
    ...
    // Each reconstructed revision knows exactly which record produced it, so
    // an incorporating review maps to its revision by identity. Two reviews
    // sharing a millisecond cannot be confused for one another.
  ...
  function update(ref, changes, author) {
    ...
    appendRevision(located.rootId, "update", snapshot, author, {});
  ...
  function comment(ref, input, author) {
    var located = requireLive(ref);
    var data = input || {};
  ...
  function link(ref, links, author) {
    ...
    appendRevision(located.rootId, "link", snapshot, author, {});
    return read(ref, false);
  }
