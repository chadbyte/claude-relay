...
function attachHomeChat(deps) {
  ...
  function transformEvent(event, mateId) {
    if (!event || typeof event.type !== "string") return null;
    if (event.type === "delta" && typeof event.text === "string") {
      return { type: "home_mate_delta", mateId: mateId, text: event.text };
    }
    if (event.type === "result" || event.type === "done") {
      return { type: "home_mate_done", mateId: mateId };
    }
    if (event.type === "error") {
      return { type: "home_mate_error", mateId: mateId, text: event.text || "Unknown error" };
    }
    return null;
  }
  ...
  function setupTap(ws, found, sessionId) {
    ...
    var unsubscribe = sessionManager.subscribeSession(sessionId, function (event) {
      ...
      var transformed = transformEvent(event, mateId);
  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type !== "home_mate_open" && msg.type !== "home_mate_send" && msg.type !== "home_mate_new_session" && msg.type !== "home_mate_close" && msg.type !== "home_mate_memory_list" && msg.type !== "home_mate_knowledge_list") {
      return false;
    }
