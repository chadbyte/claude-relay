...
function attachHomeChat(deps) {
  ...
  function transformEvent(event, mateId, session, requestId) {
    if (!event || typeof event.type !== "string") return null;
    var sessionId = session ? sessionReference(session) : null;
    var metadata = {
      mateId: mateId,
      sessionId: sessionId,
    ...
    if (event.type === "result" || event.type === "done") {
      return Object.assign({ type: "home_mate_done" }, metadata);
    }
    if (event.type === "error") {
  ...
  function setupTap(ws, found, sessionId, requestId) {
    ...
    var unsubscribe = sessionManager.subscribeSession(sessionId, function (event) {
      ...
      var transformed = transformEvent(event, mateId, activeSession, requestId);
    ...
    ws._homeChatTap = {
      ...
      sessionId: sessionId,
      sessionReference: openedSession ? sessionReference(openedSession) : null,
      mateId: mateId,
      mateSlug: found.slug,
      requestId: requestId || null,
      openedAt: Date.now(),
  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type !== "home_mate_open" && msg.type !== "home_mate_sessions_list" && msg.type !== "home_mate_session_open" && msg.type !== "home_mate_send" && msg.type !== "home_mate_new_session" && msg.type !== "home_mate_close" && msg.type !== "home_mate_memory_list" && msg.type !== "home_mate_knowledge_list" && msg.type !== "home_mate_models_get" && msg.type !== "home_mate_model_set") {
      return false;
    }
    ...
    if (msg.type === "home_mate_models_get") {
      homeModels.sendMateModels(ws, found, msg.requestId || null).catch(function (error) {
        sendMessage(ws, { type: "home_mate_models_state", mateId: found.mate.id, requestId: msg.requestId || null, vendor: found.mate.vendor || "claude", model: found.mate.model || "", models: [], status: "error", error: error.message || String(error) });
      });
      return true;
    }
