...
function attachConnection(ctx) {
  ...
  function handleConnection(ws, wsUser, handleMessage, handleDisconnection) {
    ...
    if (sm.currentModel) {
      var _activeSession = sm.activeSessionId != null ? sm.sessions.get(sm.activeSessionId) : null;
      var _activeVendor = (_activeSession && _activeSession.vendor) || sm.defaultVendor || "claude";
      sendTo(ws, { type: "model_info", model: sm.currentModel, models: sm.availableModels || [], vendor: _activeVendor, availableVendors: sm.availableVendors || [], installedVendors: sm.installedVendors || [] });
    }
    ...
    if (active && !autoCreated) {
      ...
      sendTo(ws, { type: "session_switched", id: active.localId, cliSessionId: active.cliSessionId || null, loop: active.loop || null, vendor: active.vendor || null, hasHistory: (active.history && active.history.length > 0) });
