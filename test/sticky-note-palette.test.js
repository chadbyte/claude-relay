...
test("transparent live notes isolate their blur while preserving a readable content floor", function () {
  var liveRule = css.slice(css.indexOf(".sticky-note {"), css.indexOf(".sticky-note:hover"));
  assert.match(liveRule, /-webkit-backdrop-filter: blur\(7px\) saturate\(0\.92\) contrast\(1\.02\);/);
  assert.match(liveRule, /backdrop-filter: blur\(7px\) saturate\(0\.92\) contrast\(1\.02\);/);
  assert.match(liveRule, /contain: layout style;/);
  assert.match(liveRule, /will-change: transform, backdrop-filter;/);
