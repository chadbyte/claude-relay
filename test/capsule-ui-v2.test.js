...
test("Capsule UI stylesheet is isolated, themed, accessible, and responsive", function () {
  ...
  assert.match(css, /--capsule-accent: #b96842/);
  assert.match(css, /\.dark-theme \.home-tool-display/);
  assert.match(css, /\.tool-button--variant-primary/);
  assert.match(css, /\.tool-card--variant-output/);
