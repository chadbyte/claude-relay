...
| `session-driver-eligibility.js` | **Single source of truth** for who may hold the Driver role. Per-family ordered tiers with a threshold: claude `haiku < sonnet < opus < fable`, threshold **Fable**; codex `gpt-5.2 < gpt-5.5 < luna < sol < terra`, threshold **Sol**. Matched by tier token across id, alias, `resolvedModel` and display name through `project-models.modelEntryMatches`, so a later release in the same tier stays eligible. An unrecognized model in a known family, and every other vendor, fail closed with an English reason; those vendors are still fine as Workers. Do not string-match tiers anywhere else |
| `project-pair-lifecycle.js` | Autonomous Worker management: bounded `partner_status` (context tokens and capacity ratio from `session.lastContextUsage` else the last `result` usage, activity, continuity, replace-safety — never a transcript), atomic `replace_partner`, and the per-Driver generation/evaluation ledger (`record_partner_evaluation`, bounded enum, in-memory on the live Driver session). Replacement is **transactional**: preflight runs before any interrupt/cancel/dissolve, generation closure and evaluation happen only after the new Worker exists, and a late group-write failure rolls the pair back. Re-checks exact live session identity, eligibility, owner and live pair on every call |
| `session-pair-factory.js` | Pair creation for both flows ("Add Split Worker" and Driver-initiated). **Validates everything before creating anything**, so a rejection leaves no orphan session: Worker runtime first, then an existing Driver's eligibility, or for a newly requested Driver the tier of the model it would actually resolve to. `preflightRuntime` / `preflightWorkerForDriver` are side-effect free so a caller about to destroy something can check first. An explicit model must match the vendor's populated catalog via `modelEntryMatches`; an unpopulated catalog fails closed rather than trusting caller text. Ownership comes from the connection or the Driver session, never the message |
| `session-pair-mcp-server.js` | SDK-free pair tool definitions. Base set (`send_to_partner`, `read_partner`, `interrupt_partner`, `close_partner`) is always returned; `options.lifecycle` adds `partner_status`, `replace_partner`, `record_partner_evaluation` |
| `session-pair-prompts.js` | Pure Driver-facing prompt text (`DRIVER`, `UNPAIRED`). States the management objective — protect Driver context, keep the Worker compact, reuse only when continuity beats context cost, replace when stale, choose and evaluate models — and authorizes autonomous lifecycle action. **No proposal or approval language**: edit prose here, not in the coordinator |
| `project-worker-proposal.js` | **Retired path, kept for compatibility.** `getToolDefs` returns nothing and `getSystemPrompt` returns `""`, so no model is ever told to propose a Worker; a qualified Driver creates one directly. `retiredToolDefs` and the `worker_proposal_response` handler remain only so an older client's in-flight proposal still resolves. Do not build on this |
...
| `sdk-bridge.js` → `checkToolWhitelist` | Where a tool becomes auto-approved. The pair lifecycle tools and `respond_to_worker_permission` are allowed here because prompting per hop would reinstate the approval gate the autonomous lifecycle removes; each one re-checks identity, eligibility, owner and live pair in its own handler. Accept **only the two name forms the bridge can emit**: `mcp__clay-sessions__<tool>` matched by exact prefix (never a trailing-suffix match, or another server could borrow the name), and the bare name only for tools Codex does not canonicalize itself (it canonicalizes `send_to_partner` and `read_partner`). `spawn_sessions` is deliberately excluded. When adding an entry, add the exact names — never a prefix |
...
| `worker-proposal.js` | Dormant compatibility renderer for the retired Worker proposal card. The server no longer offers `propose_worker`, so nothing new arrives here; it stays only so an older in-flight proposal still renders and resolves. Do not extend it — a qualified Driver creates its Split Worker directly (see `project-pair-lifecycle.js`) |
...
| `project-logs.js` + `project-logs-render.js` + `project-logs-ambient.js` | Project Logs ledger in the bounded right workbench pane. One navigation stack: full-pane list, full-pane entry detail, Back preserving query/filter/scroll. `project-logs-render.js` owns markup only (ledger rows, detail, discussion) and never touches the WebSocket. There is no canonical create or edit UI: people read and comment. `project-logs-ambient.js` owns ambient discovery: a slim right-edge handle that previews the pane on hover or focus and pins on any meaningful interaction, plus a restrained unread marker on the handle and toolbar button when the Driver revises the ledger. An update never opens the pane. Unread is live-session only |
