...
| `project-logs.js` + `project-logs-mcp-server.js` | `project_logs_list`, `project_log_read`, `project_log_create`, `project_log_update` | Project Logs: user-bound WS surface and the session-bound `clay-logs` MCP server. Project sessions get project-scoped tools with no `projectSlug` argument; authoritative builtin Clay gets read-only cross-project tools; ordinary Mates get none. Scope is always bound server-side, never read from the message |
...
| `project-logs-store.js` | Project Logs projection: kinds, stable opaque `log:` refs, revision chains, tombstones, author/blame, bounded list/search/read/history. Resolves a worktree to its parent project so worktree work never forks project knowledge |
...
| `project-logs-query.js` | Project Logs filtering, BM25 ranking, snippets, and pagination, extracted from `project-logs-store.js` to keep it under the size limit |
...
| `project-logs-service.js` | Project Logs authorization. Binds from exact session object identity, `ws._clayUser`, or the confirmed builtin Clay Mate registry entry; fails closed on an unattributed multi-user session; re-authorizes every call |
