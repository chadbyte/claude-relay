...
| `project-logs.js` + `project-logs-mcp-server.js` | `project_logs_list`, `project_log_read`, `project_log_comment` (`project_log_create`/`project_log_update` are retired and always refused); broadcasts `project_log_updated` | Project Logs: user-bound WS surface and the session-bound `clay-logs` MCP server. **Canonical entries are created and revised only by Project Driver sessions**; connected humans read and append attributed comments, which are proposals the Driver reviews (`list_log_feedback`, `review_log_comment`) rather than automatic changes. The system prompt carries only a pending count, never comment bodies. Guidance also makes capturing durable learning moments a default: a direct conceptual question, or a vague description the Driver gives a precise name, is recorded under the project-local `learning` category rather than being allowed to evaporate. `ATTENTION_CONTRACT` states the Sticky Notes boundary: notes are the transient attention layer and are removed once resolved, Logs are the permanent versioned record. A Driver that opens a note for a concrete defect it discovered and left unresolved must also create or update a Log entry, put that entry's opaque `log:` ref in the note, and on resolution revise the same entry with remediation/verification/outcome before removing the note. This is Driver-only guidance and never note content: creating a note does not mutate the ledger, because people and authority-less Mates write notes too. Authoritative builtin Clay gets read-only cross-project tools; ordinary Mates get none. Scope and identity are always bound server-side, never read from the message |
