...
| `session-pair-factory.js` | Pair creation for both flows ("Add Split Worker" and Driver-initiated). **Validates everything before creating anything**, so a rejection leaves no orphan session: Worker runtime first, then the existing Driver's structural capability. Any installed, available model may be the Driver. `preflightRuntime` / `preflightWorkerForDriver` are side-effect free so a caller about to destroy something can check first. An explicit model must match the vendor's populated catalog via `modelEntryMatches`; an unpopulated catalog fails closed rather than trusting caller text. Ownership comes from the connection or the Driver session, never the message |
