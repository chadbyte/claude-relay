...
## Phase 2: Project Data Store

**Goal**: Give each project a lightweight key-value data store backed by Node 22 native SQLite. Mates write data here; widgets read from here; users can inspect it transparently.
...
```
~/.clay/data/{encoded-cwd}/store.db
```
...
CREATE TABLE kv (
  ...
  updated_by TEXT               -- mate_id or 'user'
);
```

That is it. Intentionally minimal. `value` is always valid JSON.

### 2.2 Data Store Module

New module: `lib/project-datastore.js` following `attachDatastore(ctx)` pattern.

**Server API**:
```js
var ds = attachDatastore(ctx);

// Mate writes data
ds.set("monthly-expense", { total: 320000, categories: [...] }, mateId);

// Read
ds.get("monthly-expense");   // -> { key, value, updated_at, updated_by }

// List all keys
ds.keys();                    // -> ["monthly-expense", "budget-remaining", ...]

// Delete
ds.del("monthly-expense");
```

**WebSocket messages**:
| Message | Direction | Description |
|---------|-----------|-------------|
| `data_get` | client -> server | Read a key |
| `data_set` | client -> server | Write a key (from widget or user) |
| `data_keys` | client -> server | List all keys in project |
| `data_update` | server -> client | Push data change to connected clients |
| `data_delete` | client -> server | Remove a key |

**HTTP endpoints** (in `project-http.js`):
```
GET  /api/data/:key          -- Read a key (for widget HTML fetch)
PUT  /api/data/:key          -- Write a key
GET  /api/data               -- List all keys
```

Widgets (sandboxed HTML) use the HTTP endpoints. Internal code uses the module API directly.

### 2.3 SDK Tool Integration

Expose datastore to Mates via a custom SDK tool so Mates can read/write data during conversations:
...
This lets Mates update dashboard data as part of their normal task execution.

### 2.4 Data Inspector UI

Users should be able to see exactly what data exists in their project.

**Location**: Project settings or dedicated "Data" tab.

**Features**:
- List all keys with last updated time and which Mate wrote it
- Click a key to see raw JSON value (formatted)
- Edit or delete keys manually
- Search/filter keys

### 2.5 Deliverables

- [ ] `lib/project-datastore.js` module
- [ ] SQLite file management (create, open, close per project)
- [ ] WebSocket message handlers
- [ ] HTTP API endpoints
- [ ] SDK tool registration for Mates
- [ ] Data inspector UI (project settings panel)
